...
public class ElektrickaTrouba {
  ...
  public void vlozSiDovnitr(PlechNaPeceni co) {
    ...
    if (co.getPocetVajec() < 4) {
      throw new IllegalStateException("Testo na plechu nema dostatek vajec. Obsahuje " + co.getPocetVajec() + " ks");
    }
    if (co.getPocetVajec() > 4) {
      throw new IllegalStateException("Testo na plechu ma prebytek vajec. Obsahuje " + co.getPocetVajec() + " ks");
    }
    plechUvnitr = co;
    dobaPeceni = 0;
